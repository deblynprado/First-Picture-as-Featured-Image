...
function fpfi_post_type_render() { 
	...
	<select name='fpfi_settings[fpfi_post_type]'>
		<option value='post' <?php selected( $options['fpfi_post_type'], 1 ); ?>><?php _e( 'Post', 'fpfi' )?></option>
		<option value='page' <?php selected( $options['fpfi_post_type'], 2 ); ?>><?php _e( 'Page', 'fpfi' )?></option>
	</select>
<?php
}
